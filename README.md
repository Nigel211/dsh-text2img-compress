...
**压缩约 4~5×**。字号越小越密（16px 约 3000 字/页 → 约 8×），最大安全页数为 10。
...
设置项（settings 命名空间 `text2img`）：`enabled`（默认 false）、`fontPx`（默认 18）、
`threshold`（默认 600）。也可在 DSH 设置文件里直接改。
...
- 超长文本（> 10 页 ≈ 约 22000 字@18px）：插件会**自动回退纯文本**，绝不截断；
- 小图/极小字号下的复杂排版（表格、公式）。
